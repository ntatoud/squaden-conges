...
export type MultiSelectProps = ComboboxProps<OptionBase, true> &
  InputPropsRoot & {
    /** Allow user to clear the select using a button */
    withClearButton?: boolean;
    options: ReadonlyArray<NonNullable<OptionBase>>;
    inputProps?: Omit<
    ...
    renderOption?: (option: NonNullable<OptionBase>) => ReactNode;
...
export const MultiSelect = ({
  ...
  const handleOnValueChange: MultiSelectProps['onChange'] = (value) => {
    setSearch('');
    onChange?.(value);
  };
  ...
  return (
    <Combobox
      immediate
      value={value}
      onChange={(v) => handleOnValueChange(v ?? ([] as unknown as TODO))}
    ...
    >
      <div className="relative">
        ...
        <ComboboxInput<OptionBase[], typeof Input>
          as={Input}
          size={size}
          displayValue={(item) => item.map((i) => i?.label)?.join(' • ') ?? ''}
          ...
          endElement={
            <div className="flex gap-1">
              {!!withClearButton && !!value?.length && (
                <Button
                  variant="ghost"
                  size="icon-xs"
                  onClick={() => {
                    onChange?.([] as unknown as typeof value);
                  }}
                >
                  <X />
                </Button>
